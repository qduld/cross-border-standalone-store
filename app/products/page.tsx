...
const allProducts = [
  ...
  {
    id: "3",
    title: "Seashell Bracelet - Starfish Design",
    price: 35.00,
  ...
  {
    id: "4",
    title: "Seashell Ring - Pearl Accent",
    price: 25.00,
  ...
  {
    id: "6",
    title: "Butterfly Hairpin - Gold & Red",
    price: 38.00,
  ...
  {
    id: "14",
    title: "Handknit Bag - Straw Style",
    price: 48.00,
...
export default async function ProductsPage({
  ...
  const filteredProducts = category
    ? allProducts.filter(p => p.category === category || p.category.toLowerCase().includes(category))
    : allProducts;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white via-red-50/30 to-white">
      ...
      {/* Page Header */}
      <div className="bg-gradient-to-r from-red-50 to-orange-50 py-16 border-b border-red-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="text-5xl font-bold mb-4 bg-gradient-to-r from-red-700 to-orange-500 bg-clip-text text-transparent">
            {category ? `${category} Collection` : 'All Products'}
          </h1>
          <p className="text-gray-600 text-lg">
            Discover our handcrafted collection of unique jewelry and accessories
          </p>
      ...
      {/* Filters */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sticky top-16 bg-white/80 backdrop-blur-sm z-40">
        <div className="flex flex-wrap gap-4 items-center">
          <span className="text-gray-700 font-bold">Filter by:</span>
          <Link href="/products">
            <Button variant={!category ? "gradient" : "outline"} size="sm">
              All
            </Button>
          </Link>
          <Link href="/products?category=Seashell">
            <Button variant={category === "Seashell" ? "gradient" : "outline"} size="sm">
              🐚 Seashell
            </Button>
          </Link>
          <Link href="/products?category=Chinese+Style">
            <Button variant={category === "Chinese Style" ? "gradient" : "outline"} size="sm">
              🎋 Chinese Style
            </Button>
          </Link>
          <Link href="/products?category=Handknit">
            <Button variant={category === "Handknit" ? "gradient" : "outline"} size="sm">
              🧶 Handknit
            </Button>
          </Link>
        </div>
          ...
          {filteredProducts.map((product) => (
            <Link key={product.id} href={`/product/${product.id}`}>
              <div className="group relative overflow-hidden rounded-2xl bg-white shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2">
                ...
                <div className="p-6">
                  <div className="text-xs text-red-700 mb-2 font-bold">{product.category}</div>
